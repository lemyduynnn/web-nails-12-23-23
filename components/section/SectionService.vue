          ...
          <div class="text-black flex flex-col gap-6">
            ...
            <div class="flex gap-24">    
              <ul class="lg:flex-col flex-row flex lg:gap-6 lg:justify-start lg:items-start justify-between items-between w-full mb-10">
                <li v-for="(item, index) in block.listTab" :key="index" :class="{ 'bg-black text-white flex justify-between items-center gap-4 w-full': isActive(item.num) }" @click="setActive(item.num)" class="w-full text-sm px-4 py-2 cursor-pointer whitespace-nowrap text-black font-medium lg:text-[40px] text-[20px] leading-tight">
                  {{item.title_tab}}
